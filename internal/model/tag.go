...
type Tag struct {
	...
	Name          string            `gorm:"column:name;uniqueIndex;type:varchar(64);not null" json:"name" binding:"required"`        // 标签名称
	Subscriptions []SubscriptionRef `gorm:"many2many:tag_subscriptions;joinForeignKey:TagID;joinReferences:SubscriptionID;constraint:OnDelete:CASCADE" json:"-"` // 关联订阅 ID。
