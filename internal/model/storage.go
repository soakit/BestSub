...
type StorageTargetConfig struct { // 储存目标基础配置，各类型参数由 internal/storage 按 Type 解析
	Name   string          `gorm:"column:name;type:varchar(255)" json:"name" binding:"required"`                        // 显示名称
	Type   string          `gorm:"column:type;type:varchar(32);not null" json:"type" binding:"oneof=local webdav gist"` // 储存类型: local/webdav/gist
