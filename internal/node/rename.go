...
type templateData struct { // 重命名模板的渲染上下文。
	...
	Sub               model.Subscription // 节点的订阅来源信息，单独节点时字段均为零值。
...
func Rename(node Node, index uint32, expression string) (string, error) {
	...
	subscription, _ := store.SubscriptionGet(node.SubscriptionID)
	var result bytes.Buffer
	if err := tmpl.Execute(&result, templateData{
		Index:             index,
		Sub:               subscription,
		Delay:             uint32(node.Info.Delay),
		DownloadSpeed:     node.Info.DownloadSpeed,
		TrafficMultiplier: node.Info.TrafficMultiplier,
		Country:           countries.Get(node.Info.CountryCode),
	}); err != nil {
